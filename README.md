...
* If you're using [Preact](https://preactjs.com/) instead of React, you'll need to configure the JSX factory. You can either pass `--jsx-factory=h --jsx-fragment=Fragment` to esbuild on the command line, or add `"jsxFactory": "h", "jsxFragmentFactory": "Fragment"` to your `tsconfig.json` file. You will also have to add `import {h, Fragment} from 'preact'` in files containing JSX syntax.
